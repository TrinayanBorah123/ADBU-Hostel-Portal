<?php
	...
	if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		$sid=$row['sid'];
		$status="Pending";
		$rs=mysqli_query($con,"insert into renew_t value(default,'$renewdate','$rsem','$status','$sid')");
	  	if($rs)
 		{   
...
<body>
 <nav class="navbar fixed-top navbar-expand-lg  navbar-light bg-light">
	...
	<center>
	 <div style="box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);transition: 0.3s;width: 40%;height:540px;border-radius: 5px; ">
	<div style="margin-top:160px;">
	 <p id="header" style="text-align:center;font-size: 35px;font-family: Consolas;">Renew Request</p>
	<hr/>
	<?php 
		$sql = "SELECT * FROM student WHERE sid = '".$_SESSION["login_user"]."'";
    	$result = mysqli_query($con,$sql);
	...
	<div>
  	<form style="word-spacing: 50px;margin-top: 20px" action="" method="POST">
		  <input  style="margin-left: 10px;margin-top:20px" type="number" id="rsem" name="rsem" placeholder="Last Semester" required></br>
		 <input  style="margin-left: 10px;margin-top:20px;border: 2px solid #000000" type="date" id="renewdate" name="renewdate" placeholder="Renew Date" required>

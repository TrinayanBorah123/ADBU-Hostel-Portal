...
<body>
 ...
 <div style="box-shadow: 0 4px 8px 0 rgba(0,0,0,0.2);transition: 0.3s;width: 40%;height:490px;border-radius: 5px; ">
	...
	<div style="margin-top:160px;background-color:#6c757d;color:#ffffff">
	 <p style="text-align:center;font-size: 35px;font-family: Fabric;">Security Login</p>
	<hr/>
	</div >
	<div >
	
  	<form style="word-spacing: 50px;margin-top: 80px" action="" method="POST">
		 <input  style="margin-left: 10px" type="text" name="id" placeholder="Security ID" required></br>
	...
	<p style="margin-top:20px; "><a href="securityregister.php" style="text-decoration:none ">Do not have an account?<i class="fas fa-user-plus"></i></a></p>
	</div>
	 </div>
	</center>
   ...
   // include 'session.php';
	...
	if(!$db_selected)
	{
     ...
     if(isset($_POST['id'])){
      ...
      $sql = "SELECT userid FROM security WHERE userid = '".$myuserid."' and password = '".$mypassword."'";
      $result = mysqli_query($con,$sql);
    
    
      $count = mysqli_num_rows($result);
      if($count == 1) {
   		$_SESSION["login_security"] = $myuserid;
        //header("location:securityDash.php");
		echo '<script>window.location.href = "securityDash.php";</script>';
